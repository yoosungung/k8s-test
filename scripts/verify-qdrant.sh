...
if [[ -n "${node_ip}" ]]; then
  if curl -fsS -m 5 -H "api-key: ${QDRANT_API_KEY}" \
    "http://${node_ip}:${QDRANT_REST_PORT}/collections" >/dev/null 2>&1; then
    ok "External REST on ${node_ip}:${QDRANT_REST_PORT} is reachable"
  else
    warn "External REST on ${node_ip}:${QDRANT_REST_PORT} not reachable from this host (may be expected off-cluster)"
  fi
fi
